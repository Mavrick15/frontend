...
(function () {
  if (window.__silktideConsentManagerLoaded) return;
  window.__silktideConsentManagerLoaded = true;
  ...
  class SilktideCookieBanner {
    ...
    // ----------------------------------------------------------------
    // Sanitize HTML to reduce XSS risk before innerHTML
    // ----------------------------------------------------------------
    sanitizeHtml(html) {
    ...
    createWrapperChild(htmlContent, id) {
      ...
      if (htmlContent != null && typeof htmlContent === "string") {
        child.innerHTML = this.sanitizeHtml(htmlContent);
      }
    ...
    shouldShowBackdrop() {
      const show = this.config?.background?.showBackground;
      return Boolean(show);
    }
    ...
    getBannerContent() {
      ...
      const acceptAllAria =
        acceptAllButtonLabel && acceptAllButtonLabel !== acceptAllButtonText
          ...
          : "";
      const acceptAllButton = `<button class="accept-all st-button st-button--primary"${acceptAllAria}>${acceptAllButtonText}</button>`;
      ...
      const rejectNonEssentialAria =
        rejectNonEssentialButtonLabel &&
        rejectNonEssentialButtonLabel !== rejectNonEssentialButtonText
          ? ` aria-label="${rejectNonEssentialButtonLabel}"`
          : "";
      const rejectNonEssentialButton = `<button class="reject-all st-button st-button--primary"${rejectNonEssentialAria}>${rejectNonEssentialButtonText}</button>`;
    ...
    getModalContent() {
      ...
      const preferencesButtonLabel =
        this.config.text?.banner?.preferencesButtonAccessibleLabel;
      const closeButtonAria = preferencesButtonLabel
        ? ` aria-label="${preferencesButtonLabel}"`
        : "";

      const closeModalButton = `<button class="modal-close"${closeButtonAria}>
      ...
      const acceptAllButtonLabel =
        this.config.text?.banner?.acceptAllButtonAccessibleLabel;
      const modalAcceptAria =
        acceptAllButtonLabel && acceptAllButtonLabel !== acceptAllButtonText
          ? ` aria-label="${acceptAllButtonLabel}"`
          : "";
      const acceptAllButton = `<button class="preferences-accept-all st-button st-button--primary"${modalAcceptAria}>${acceptAllButtonText}</button>`;
      ...
      const rejectNonEssentialButtonLabel =
        this.config.text?.banner?.rejectNonEssentialButtonAccessibleLabel;
      const modalRejectAria =
        rejectNonEssentialButtonLabel &&
        rejectNonEssentialButtonLabel !== rejectNonEssentialButtonText
          ? ` aria-label="${rejectNonEssentialButtonLabel}"`
          : "";
      const rejectNonEssentialButton = `<button class="preferences-reject-all st-button st-button--primary"${modalRejectAria}>${rejectNonEssentialButtonText}</button>`;
    ...
    createCookieIcon() {
      ...
      this.cookieIcon.appendChild(this.createCookieIconSvg());
