...
const BlogPreview = () => {
  ...
  const mobilePostCards = latestPosts.map((post) => (
  ...
  const desktopPostCards = latestPosts.map((post) => (
    <BlogPostCard
      key={post.id}
      title={post.title}
      excerpt={post.excerpt}
  ...
  const stepIndicators = [0, 1, 2].map((i) => (
    <div
      key={`indicator-${i}`}
  ...
  return (
    <section
      id="blog"
      className="py-12 md:py-24 px-4 md:px-12 bg-gradient-to-br from-white via-gray-50 to-gray-100"
    >
      <div className="container mx-auto max-w-6xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 md:mb-12">
            ...
            <div className="flex items-center gap-2 mb-4">
              <div className="p-2 bg-gray-100 rounded-lg">
                <Newspaper
                  ...
                  aria-label={TEXT_CONSTANTS.SECTION_TITLE_ICON_ALT}
                />
              </div>
          ...
          <Link to="/blog" className="mt-4 md:mt-0">
            <Button
              variant="outline"
              className="group border-gray-900 text-gray-900 hover:bg-gray-900 hover:text-white shadow-lg hover:shadow-xl transition-all duration-300 px-6 py-6 rounded-xl font-semibold"
            >
