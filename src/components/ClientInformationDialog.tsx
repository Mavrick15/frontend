...
const ClientInformationDialog = () => {
  ...
  const [isInvoiceGeneratedDialogOpen, setIsInvoiceGeneratedDialogOpen] =
    useState(false);
  ...
  const [generatedInvoiceItems, setGeneratedInvoiceItems] = useState<
    CartItem[]
  >([]);
  ...
  const validateForm = (): boolean => {
    ...
    const finalFullName = fullName?.trim() || userName;
    ...
    const finalEmail = email?.trim() || userEmail;
    if (!finalEmail.trim()) {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      toast.error(
        `${missingFields} champ(s) requis manquant(s). Veuillez remplir tous les champs obligatoires.`,
      );
    }
  ...
  const handleGenerateInvoice = async () => {
    ...
    try {
      ...
      const msg =
        error instanceof Error ? error.message : getNetworkErrorMessage();
  ...
  return (
    <>
      <Dialog
        ...
        onOpenChange={setIsClientInfoDialogOpen}
      >
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3 mb-4 overflow-y-auto max-h-[120px] pr-2 custom-scrollbar">
                    {cartItems.map((item, index) => {
                      ...
                      const linePrice = Number.isFinite(rawPrice)
                        ? rawPrice * (item.quantity ?? 1)
                        : 0;
                      return (
                        <motion.div
                          key={item._id}
