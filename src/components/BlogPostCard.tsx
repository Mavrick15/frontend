...
const TEXT_CONSTANTS = {
  PLACEHOLDER_IMAGE_URL: (category: string) =>
    `https://placehold.co/400x200/1a1a1a/ffffff?text=${encodeURIComponent(category)}`,
  IMAGE_ALT: (title: string, category: string) =>
...
const BlogPostCard = (props: BlogPostCardProps) => {
  const { title, excerpt, imageUrl, date, slug, category } = props;
  return (
    <Link to={`/blog/${slug}`} className="h-full block group">
      <Card className="overflow-hidden hover:shadow-2xl transition-all duration-300 h-full rounded-2xl border border-gray-200/50 bg-white/90 backdrop-blur-sm hover:-translate-y-2 flex flex-col">
        <div className="relative h-48 overflow-hidden">
          <img
            ...
            onError={(e) => {
              ...
              e.currentTarget.src =
                TEXT_CONSTANTS.PLACEHOLDER_IMAGE_URL(category);
            }}
          />
        ...
        <CardContent className="p-6 flex flex-col flex-1">
          <p className="text-gray-600 text-sm mb-3 font-semibold">{date}</p>
          ...
          <Button
            variant="outline"
            className="group mt-auto bg-gray-900 text-white border-0 hover:bg-gray-800 shadow-lg hover:shadow-xl transition-all duration-300"
          >
